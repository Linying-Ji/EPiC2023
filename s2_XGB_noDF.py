...
def get_rmse_save_result (trials, best, X_train):
  ...
  joblib.dump(best_model, 'Model_s2_XGB_noDF_allfold_'+condition+'.joblib')
  ...
  result.to_csv('Hyper_s2_XGB_noDF_allfold_'+condition+'.csv', index=False)
...
file_list = ["train_scenario_2_fold_0.csv", 
             "train_scenario_2_fold_1.csv", 
             "train_scenario_2_fold_2.csv", 
             "train_scenario_2_fold_3.csv",
             "train_scenario_2_fold_4.csv"]
...
save_path = "XXX/EPiC_2023/Modeling/XGBoost/s2/" 
os.chdir(save_path)

###### Hyperopt #######--------------------------------------------------------
max_eval = 50 # number of iterations in hyperopt.

# Step 1: Initialize space for exploring hyperparameters:
space={
        'max_depth': hp.quniform("max_depth", 3, 10, 1), #hp.choice('max_depth', np.arange(3, 12, dtype=int)), 

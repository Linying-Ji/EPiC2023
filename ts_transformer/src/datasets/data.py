...
class EpicData(BaseData):
    """
    Dataset class for Epic dataset.
    Attributes:
        all_df: dataframe indexed by ID, with multiple rows corresponding to the same index (sample).
            Each row is a time step; Each column contains either metadata (e.g. timestamp) or a feature.
        feature_df: contains the subset of columns of `all_df` which correspond to selected features
    ...
    def __init__(self, root_dir=None, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        df = pd.read_csv('./train_scenario_3_all_fold.csv',dtype='float32')
...
class EpicDataTest(BaseData):
    ...
    def __init__(self, root_dir=None, file_list=None, pattern=None, n_proc=1,
                 limit_size=None, config=None):

        self.set_num_processes(n_proc=n_proc)

        df = pd.read_csv('./test_scenario_2_fold_4_dynfeatures.csv', dtype='float32')
...
data_factory = {'weld': WeldData,
                ...
                'pmu': PMUData,
                'epic': EpicData, 'epictest': EpicDataTest}
